...
//! consumers (crossplane-forge → a downstream vendor provider) depend on this output
...
pub struct GoFile {
    ...
    /// a downstream vendor provider). Each emitter sets its own tool name here.
...
mod tests {
    ...
    fn composite_literal_with_named_fields() {
        let expr = GoExpr::Composite {
            ty: GoType::qualified("vendor", "AuthMethod"),
            fields: vec![
        ...
        file.decls.push(GoDecl::Func(GoFuncDecl {
            ...
            recv: None,
            params: vec![],
            returns: vec![GoType::qualified("vendor", "AuthMethod")],
            body,
        }));
        let s = render(&file);
        assert!(s.contains("return vendor.AuthMethod{"));
    ...
    fn package_level_groupname_marker_above_package_decl() {
        ...
        f.markers.push(KubeMarker::GroupName("vendor.crossplane.io".to_string()));
        let s = render(&f);
        // Markers come before package
        let pkg_pos = s.find("package v1alpha1").unwrap();
        let marker_pos = s.find("// +groupName=vendor.crossplane.io").unwrap();
